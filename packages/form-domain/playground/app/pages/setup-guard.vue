...
/**
 * Fixture de tipos do formato setup. Toda garantia daqui já existia no builder;
 * o ponto desta página é provar que nenhuma se perdeu na travessia.
 */
...
const domain = defineFormDomain('setup-guard-domain', { title: 'Guarda', order: 1 }, () => {
...
/** Metadata é opcional. */
const semMeta = defineFormDomain('setup-guard-sem-meta', () => ({
...
<template>
  <div>{{ tipo }} {{ parcial }} {{ completo }} {{ titulo }} {{ gritado }} {{ projetado }} {{ nome }}</div>
</template>
